			...
			<div class="header_inner">
				<div class="logo_box">
					<a href="" class="logo_link">
						<img src="${uploadUrlPath}현이의가방끈.png" alt="로고">
					</a>	
				</div>	
			</div>	
		</header>
							        ...
							        <div class="pw_valid_wrap">
										...
										<ul class="pw_valid_list">
										    ...
										    <li v-if="!isPasswordValidForLength" class="pw_valid_item" 
										        :style="{ 
										            backgroundImage: isPasswordValidForLength ? 'url(${uploadUrlPath}green-check.png)' : 'url(${uploadUrlPath}check.png)',
										    ...
										    <li v-if="!isPasswordValidForNoSpaces" class="pw_valid_item" 
										        :style="{ 
										            backgroundImage: isPasswordValidForNoSpaces ? 'url(${uploadUrlPath}green-check.png)' : 'url(${uploadUrlPath}check.png)',
										    ...
										    <li v-if="isPasswordValid" class="pw_valid_item" 
										        :style="{ 
										            backgroundImage: 'url(${uploadUrlPath}green-check.png)',
										            color: 'green' 
										        }">
										        사용 가능한 비밀번호입니다.
										    </li>
    ...
    const app = Vue.createApp({
        ...
        methods: {			
            fnGetList(){
				...
				$.ajax({
					url:"${pageContext.request.contextPath}/join.dox",
					dataType:"json",	
					type : "POST", 
					data : nparmap,
			...
			fnSave(){
				...
				$.ajax({
					url:"${pageContext.request.contextPath}/joinadd1.dox",
					dataType:"json",	
					type : "POST", 
					data : nparmap,
					success : function(data) { 
					...
					if (self.file) {
						  ...
						  $.ajax({
								url: '${pageContext.request.contextPath}/JoinfileUpload.dox',
								type: 'POST',
								data: formData,
								processData: false,  
								contentType: false,  
								success: function() {
								  ...
								  location.href = "${pageContext.request.contextPath}/intro";
								  self.filePreview = "";
								},
		...
		fnCheckuserId(){
			...
			$.ajax({
				url:"${pageContext.request.contextPath}/joinMultiCheck1.dox",
				dataType:"json",	
				type : "POST", 
				data : nparam,
				success : function(data) {
		...
		fnCheckNickName() {
		     ...
		     $.ajax({
		         url: "${pageContext.request.contextPath}/nickNameMultiCheck1.dox",
		         dataType: "json",
		         type: "POST",
		         data: nparam,
		         success: function(data) {
		 ...
		 fnAddr() {
			...
			new daum.Postcode({
		        oncomplete: function(data) {
					...
					$.ajax({
		 				url:"${pageContext.request.contextPath}/addAddress1.dox",
		 				dataType:"json",	
		 				type : "POST", 
		 				data : nparam,
		 				success : function(data) { 
